...
angular.module(window.PROJACT_Name).directive('angucomplete', function ($rootScope, $parse, $state, $sce, $timeout, fetchService, CONFIG) {
    return {
        restrict: 'EA',
        scope: {
            "id": "@id",
        ...
        link: function ($scope, elem, attrs) {
            ...
            $scope.searchTimerComplete = function (str, id) {
                ...
                var platform = $rootScope.commParams.keyTwo;
                if (id == 'phone_model') {
                    platform = $('#phone_manufacturer_value').val();
                }
                ...
                if (!$scope.autocomplete[id][first]) {
                    var searchObject = {
                        field: id,
                        keyOne: str,
                        keyTwo: platform
                    ...
                    fetchService.get(fetchArr).then(function (data) {
                        data = data[0].data;
                        if (data.error_code - 0 === 101) {
                            return $rootScope.poplayer = {
                                type: 'error',
                                content: '无数据!!'
                            };
                        }
                        $scope.autocomplete[id][first] = data.data;
                        autocompleteData(data.data, str);
                    });
                } else {
                    autocompleteData($scope.autocomplete[id][first], str);
                }
            };
